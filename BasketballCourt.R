...
# Loading the Data 
#df_season_stats <- read.csv("basketball/cdc-basketball/Seasons_Stats.csv", header = TRUE)
setwd("C:/Users/Jiin Son/Desktop/hackathon/carolinadata-basketball")
df_shots <- read.csv("NBA_Shots_2000_to_2018.csv")

#df_shots <- as.data.frame(df_shots)
#head(df_season_stats)
head(df_shots)
...
colnames(df_shots) <- c("id", "player", "year", "top_px_loc", "left_px_loc", "date", "team", "opponent", 
                        "location","quarter", "game_clock", "outcome", "shot_value", "shot_dist", "team_score",
                        "opp_score")

#Converting the factors to numerics 
df_shots$top_px_loc <- as.numeric(df_shots$top_px_loc)
df_shots$left_px_loc <- as.numeric(df_shots$left_px_loc)

#Plotting elementary plot
library(ggplot2)
library(gganimate)
plot1 <- P + 
  stat_summary_2d(data = df_shots, aes(x=top_px_loc, y=left_px_loc, z=outcome), alpha=0.8, binwidth=c(25/(2*25), 50/(2*47)), fun=sum) +
  scale_fill_viridis(option='inferno', end=1, labels=comma, trans='log10') +
  labs(title=sprintf('Heat Map of %s Basketball Shots from NCAA Games', df_shots %>% pull(outcome) %>% sum() %>% comma()),
       subtitle='Starting with the 2013-14 season. Via Sportradar data in BigQuery',
       fill='# of 2pt/3pt Shot Attempts\nMade From Spot',
       caption = "Research Pyangle") +
  bb_theme + transition_time(year)

labs(title = 'Year: {frame_time}', x = 'GDP per capita', y = 'life expectancy') +
  transition_time(year) +
  ease_aes('linear')

#Show the results of plot1
plot1
